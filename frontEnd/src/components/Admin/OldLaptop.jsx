...
const OldLaptop = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      data.append("discount", Number(form.discount));
      data.append("description", form.description);
      data.append("subcategory", activeTab);
      data.append("processorType", form.processorType || "");
      data.append("ram", form.ram || "");
      data.append("disk", form.disk || "");
      data.append("generation", form.generation || "");
      if (form.image) data.append("image", form.image);

      let res;
      if (editIndex !== null) {
        const productId = products[activeTab][editIndex]._id;
        res = await axios.put(`${BASE_URL}/api/laptop/${productId}`, data, {
        ...
        res = await axios.post(`${BASE_URL}/api/laptop`, data, {
          headers: { "Content-Type": "multipart/form-data", Authorization: `Bearer ${token}` },
        });
  ...
  const handleEdit = (product, index) => {
    setForm({
      ...
      image: null,
      preview: `${BASE_URL}/uploads/laptop/${product.image}`,
    });
    setEditIndex(index);
    setIsModalOpen(true);
  };
  ...
  const handleDelete = (index) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete(`${BASE_URL}/api/laptop/${productId}`, {
  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen">
      ...
      <div className="flex justify-center gap-4 mb-8">
        {["dell", "hp", "apple", "asus", "acer", "lenovo", "sumsung"].map(
          (tab) => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`px-6 py-2 rounded-full text-lg font-medium transition-all shadow ${activeTab === tab
      ...
      >
        ...
        <div className="mt-6">
          {products[activeTab].length === 0 ? (
            <p className="text-gray-500">No laptops added yet.</p>
          ) : (
              ...
              {products[activeTab].map((p, index) => (
                ...
                >
                  <div className="relative overflow-hidden group rounded-xl">
                    ...
                    <div className="absolute top-full left-0 group-hover:top-0 transition-all duration-500 w-full h-full bg-gradient-to-t from-black/80 to-transparent flex items-end rounded-t-xl px-4 py-3">
                      <div className="text-white text-sm space-y-1">
                        <p>PROCESSOR: {p.processorType}</p>
                        <p>HARD DISK: {p.disk}</p>
                        <p>RAM: {p.ram}</p>
